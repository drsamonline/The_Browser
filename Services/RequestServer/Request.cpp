...
size_t Request::on_data_received(void* buffer, size_t size, size_t nmemb, void* user_data)
{
    ...
    // Aetheris: Network Shield - Block ads/trackers at socket layer before processing
    if (Aetheris::is_blocked_script_url(request.url().to_string())) {
        dbgln_if(REQUESTSERVER_WIRE_DEBUG, "Aetheris Shield: Blocked tracker/ad URL: {}", request.url());
        return CURL_WRITEFUNC_ERROR; // Abort the transfer immediately
    }
    ...
    // Aetheris: Secondary content inspection - scan for inline script tags with tracking code
    if (!Aetheris::is_blocked_script_url(request.url().to_string())) {
        StringView content_view { static_cast<char const*>(buffer), total_size };
        if (content_view.contains("<script"sv) && 
            (content_view.contains("googlesyndication"sv) || 
             content_view.contains("google-analytics"sv) ||
             content_view.contains("fbq('track'"sv) ||
             content_view.contains("gtag('config'"sv))) {
            dbgln_if(REQUESTSERVER_WIRE_DEBUG, "Aetheris Shield: Blocked inline tracking script in response from {}", request.url());
            return 0; // Accept but discard this chunk
        }
    }
