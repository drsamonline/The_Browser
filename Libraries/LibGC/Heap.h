...
public:
    ...
    // Aetheris: Aggressive heap reclamation for closed tabs/background contexts
    void collect_garbage_aggressive();  // Immediate full GC with memory wipe
    void register_tab_context(void* context_id);  // Track tab/window contexts
    void unregister_tab_context(void* context_id);  // Trigger immediate GC for closed tab
...
private:
    ...
    // Aetheris: Tab context tracking for aggressive GC
    HashTable<void*> m_tab_contexts;  // Track active tab/window heap contexts
};
