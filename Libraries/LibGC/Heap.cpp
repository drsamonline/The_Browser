...
// Aetheris: Aggressive garbage collection with immediate memory reclamation for closed tabs
...
// Aetheris: Register a tab/window context for tracking
void Heap::register_tab_context(void* context_id)
{
    if (context_id) {
        m_tab_contexts.set(context_id);
        dbgln_if(false, "Aetheris: Registered tab context {}", context_id);
    }
}

// Aetheris: Unregister tab context and trigger immediate GC
void Heap::unregister_tab_context(void* context_id)
{
    if (context_id) {
        m_tab_contexts.remove(context_id);
        dbgln("Aetheris: Tab context closed {}, triggering aggressive GC", context_id);
        
        // Immediately reclaim memory for closed tab
        collect_garbage_aggressive();
    }
}
